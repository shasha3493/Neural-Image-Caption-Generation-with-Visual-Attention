    ...
    for i, (image, caps, caplens, allcaps) in enumerate(
        ...
        img_captions = list(
            map(lambda c: [w for w in c if w not in {word_map['<start>'], word_map['<end>'], word_map['<pad>']}],
                img_caps)) # remove <start>, <stop> and <pad>
        references.append(img_captions)

        # Hypotheses
        hypotheses.append([w for w in seq if w not in {word_map['<start>'], word_map['<end>'], word_map['<pad>']}]) # remove <start>, <stop> and <pad>
        ...
        # However, during test, we don't have true caption lengths. Therefore, we generate a sequence till it 
        # produces <end>. Now while calcultaing bleu score, we also, remove <end> from hypothesis and reference. 
